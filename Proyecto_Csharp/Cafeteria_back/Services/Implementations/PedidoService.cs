...
namespace Cafeteria_back.Services.Implementations
{
    public class PedidoService : IPedidoService
    {
        ...
        public PedidoService(MiDbContext context, ICarritoService carritoService, DescuentoStrategyContext descuentoContext)
        {
            _context = context;
        ...
        public async Task<object?> ObtenerMisPedidosAsync(long clienteId)
        {
        ...
        public async Task<object?> ConfirmarPedidoAsync(string carritoId, string tipoEntrega, string tipoPago)
        {
            ...
            try
            {
                ...
                if (carrito == null)
                    throw new Exceptions.NotFoundException("Carrito no encontrado.");

                if (carrito.Items == null || !carrito.Items.Any())
                    throw new Exceptions.BadRequestException("El carrito no tiene productos.");

                if (!Enum.TryParse<Tipo_entrega>(tipoEntrega, out var tipoEntregaParsed))
                    throw new Exceptions.BadRequestException("Tipo de entrega inválido.");
                ...
                foreach (var item in carrito.Items)
                {
                    ...
                    if (!productoExiste)
                        throw new Exceptions.BadRequestException($"El producto con ID {item.ProductoId} no existe.");
                    ...
                    foreach (var extra in item.Extras)
                    {
                        ...
                        if (!extraExiste)
                            throw new Exceptions.BadRequestException($"El extra con ID {extra.ExtraId} no existe.");
        ...
        public async Task<object?> ObtenerMisVentasAsync(long clienteId)
        {
        ...
        public async Task<object?> TodasLasVentasAsync()
        {
            var ventas = await _context.Ventas
                .Include(v => v.Pedido)
                .OrderByDescending(v => v.Ven_fecha)
        ...
        public async Task<object?> ObtenerTodosLosPedidosAsync()
        {
            var pedidos = await _context.Pedidos
        ...
        public async Task<object?> CambiarEstadoPedidoAsync(long pedidoId, string nuevoEstado, long? empleadoId)
        {
            if (!Enum.TryParse<Estado_pedido>(nuevoEstado, out var estadoParseado))
                throw new Exceptions.BadRequestException("Estado no válido.");

            var pedido = await _context.Pedidos.FindAsync(pedidoId);
            if (pedido == null)
                throw new Exceptions.NotFoundException("Pedido no encontrado.");

            var tipoEntrega = pedido.Tipo_Entrega;

            var transicionesValidas = tipoEntrega switch
            {
                Tipo_entrega.Mesa => new[] { Estado_pedido.Preparando, Estado_pedido.Listo, Estado_pedido.Entregado },
                Tipo_entrega.Llevar => new[] { Estado_pedido.Preparando, Estado_pedido.Listo, Estado_pedido.Entregado },
                Tipo_entrega.Delivery => new[] { Estado_pedido.Preparando, Estado_pedido.Listo, Estado_pedido.Delivery },
                _ => Array.Empty<Estado_pedido>()
            };

            if (!transicionesValidas.Contains(estadoParseado))
                throw new Exceptions.BadRequestException("Transición de estado no válida para el tipo de entrega.");

            if (pedido.Venta == null && empleadoId != null)
            {
        ...
        public async Task<object?> ObtenerInformacionCompletaDeTodosLosPedidosAsync()
        {
            var pedidos = await _context.Pedidos
                .Include(p => p.Cliente)
                .Include(p => p.Detalle_pedido)!
